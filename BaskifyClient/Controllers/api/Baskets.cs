...
namespace BaskifyClient.Controllers
{
    ...
    public class Baskets : Controller
    {
        ...
        public ActionResult results([FromHeader] string authorization)
        {
            ...
            var results = _context.BasketModel.Include(b => b.photos).Join(
        ...
        {
            var isHost = !Request.HttpContext.Request.IsLocal();
            UserModel user = null;
            BasketModel basket;
            if (isHost) //no restrictions if local
            {
                if (authorization == null)
                    return Unauthorized("No authorization");
                user = LoginUtils.getUserFromToken(authorization.Replace("Bearer ", string.Empty), _context);
                ...
                basket = _context.BasketModel.Include(b => b.AuctionModel).Where(b => b.BasketId == id).FirstOrDefault();
                if (basket == null)
                    return BadRequest("Invalid Basket");


                //if the user is nto the auction host, winner, or the submitter (if submitter delivers), don't allow
                if (!(basket.WinnerUsername == user.Username || isHost || (basket.SubmittingUsername == user.Username && basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter)))
            ...
            if (string.IsNullOrWhiteSpace(basket.TrackingNumber))
            {
                ...
                return NotFound(new { Error = "No tracking number provided", Editable = !basket.Delivered && (isHost || (basket.SubmittingUsername == user.Username && basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter)) });
            }
            ...
            try
            {
                ...
                dto.Editable = !dto.Delivered && (isHost || (basket.SubmittingUsername == user?.Username && basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter));
            ...
            catch (Exception e)
            {
                return StatusCode(426,
                    ...
                    {
                        Error = e.Message,
                        Editable = !basket.Delivered && (isHost || (basket.SubmittingUsername == user?.Username && basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter))
                    });
            }
        }
        ...
        {
            var isHost = Request.IsLocal();

            if (authorization == null && !isHost)
                return Unauthorized("No authorization");
            var user = LoginUtils.getUserFromToken(authorization.Replace("Bearer ", string.Empty), _context);
            if (user == null && !isHost)
                return Unauthorized("Bad authorization");
            ...
            //if the user is nto the auction host, winner, or the submitter (if submitter delivers), don't allow
            if (!(isHost || (basket.SubmittingUsername == user.Username && basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter)))
        ...
        public ActionResult addDispute([FromRoute] int id, [FromHeader] string authorization, [FromForm] string message)
        {
            ...
            var basket = _context.BasketModel.Include(b => b.SubmittingUser).Where(b => b.BasketId == id).FirstOrDefault();
            if (basket == null)
                return BadRequest("Invalid Basket");
            ...
            EmailUtils.SendStyledEmail(OrganizationAccount.ContactEmail, "Basket Delivery Disputed", $"The user {basket.WinnerUsername} disputed the delivery of your basket entitled \"{basket.BasketTitle}\" which was marked as delivered on {basket.DeliveryTime.Value} (UTC). <br><br> The user's dispute states: \"{basket.DisputeReason}\". <br><br> You will not be able to request a payout or create other auctions until this dispute is resolved. <br><br> Please reach out to the user at {basket.Winner.Email} to resolve the issue and have them close the dispute. {(basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter? "The basket submitter has also been notified.": "")}", _env, basket.AuctionModel.HostUser.ContactEmail);
            if(basket.AuctionModel.DeliveryType == DeliveryTypes.DeliveryBySubmitter && basket.SubmittingUsername != null)
            {
